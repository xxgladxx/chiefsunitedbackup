...
class Hunting(commands.Cog):
    ...
    async def start(self, ctx, channel: discord.TextChannel = None):
        ...
        if channel.id in channel_list:
            ...
            message = f"The hunt has started in {channel.mention}. Good luck to all. <@&828944470631514122>"
            await self.config.guild(ctx.guild).channels.set(channel_list)

        await ctx.send(bold(message), allowed_mentions=discord.AllowedMentions(roles=True))
