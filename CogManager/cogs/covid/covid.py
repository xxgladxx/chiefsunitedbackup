...
class Covid(commands.Cog):
    ...
    async def covid(self, ctx, *, country: typing.Optional[str]):
     if ctx.channel.id == 760484123709014048:
        ...
        embed.set_image(url="https://media.discordapp.net/attachments/822428936431730698/831595107156361246/tenor_1.gif")
        await ctx.send("{}, I now know that you're the one trying to spread Covid-19 in our server.\nDon't try that ever again! :microbe: :syringe: ".format(ctx.author.mention))
        await ctx.send(embed=embed)
    @covid.command()
    @commands.bot_has_permissions(embed_links=True)
