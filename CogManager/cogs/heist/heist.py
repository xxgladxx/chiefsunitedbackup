...
class Heist(commands.Cog):
    ...
    async def _targets_heist(self, ctx):
     if ctx.channel.id == 822461601160495124:
    ...
    async def _play_heist(self, ctx):
     if ctx.channel.id == 822461601160495124:
        ...
        await ctx.send("Sorry {}, you cannot begin a heist here. Please try again in <#822461601160495124>.\nThank you".format(ctx.author.mention))
        return
    ...
    async def heist_game(self, ctx, guild, t_heist, t_crew, t_vault):
        ...
        await ctx.send("Get ready! The {0} is starting with {1}\nThe {2} has decided to "
                           "hit **{3}**.".format(t_heist, start_output, t_crew, target))
